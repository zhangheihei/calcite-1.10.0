...
public class RexShuttle implements RexVisitor<RexNode> {
  ...
  public RexNode visitCall(final RexCall call) {
    boolean[] update = {false};
    //遍历所有的operand,只有子节点有一个更改过，那么整体就要修改
  ...
  protected List<RexNode> visitList(
    ...
    for (RexNode operand : exprs) {
      ...
      if ((clonedOperand != operand) && (update != null)) {
        update[0] = true;
        System.out.printf("edwin RexShuttle visitList true:operand:%s, clonedOperand:%s,"
                        + "operand class is %s, clonedOperand class is %s\n",
                operand.toString(), clonedOperand.toString(),
                operand.getClass(), clonedOperand.getClass());
      } else {
        System.out.printf("edwin RexShuttle visitList false:operand:%s, clonedOperand:%s,"
                        + "operand class is %s, clonedOperand class is %s \n",
                operand.toString(), clonedOperand.toString(),
                operand.getClass(), clonedOperand.getClass());
      }
