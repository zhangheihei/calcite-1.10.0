...
public abstract class ReduceExpressionsRule extends RelOptRule {
  ...
  public static class ProjectReduceExpressionsRule extends ReduceExpressionsRule {
    ...
    @Override public void onMatch(RelOptRuleCall call) {
      ...
      System.out.printf("edwin ReduceExpressionsRule project is %s, class is %s \n",
              project.toString(), project.getClass());

      RelNode input = project.getInput();
      if (input instanceof HepRelVertex) {
        System.out.printf("edwin ReduceExpressionsRule project input is %s, class is %s \n",
                ((HepRelVertex) input).getCurrentRel().toString(),
                ((HepRelVertex) input).getCurrentRel().getClass());
      } else {
        System.out.printf("edwin ReduceExpressionsRule project input is %s, class is %s \n",
                project.getInput().toString(), project.getInput().getClass());
      }
...
//              project.getInput().toString(), project.getInput().getClass());
      ...
      for (RexNode rex: expList) {
        System.out.printf("edwin ReduceExpressionsRule project RexNode is %s, %s, kind id %s\n",
                rex.toString(), rex.getClass(), rex.getKind());

      }
      System.out.printf("edwin ReduceExpressionsRule predicates pulledUpPredicates is %s\n",
              predicates.pulledUpPredicates);
      System.out.printf("edwin ReduceExpressionsRule predicates leftInferredPredicates is %s\n",
              predicates.leftInferredPredicates);
      System.out.printf("edwin ReduceExpressionsRule predicates rightInferredPredicates is %s\n",
              predicates.rightInferredPredicates);

      if (reduceExpressions(project, expList, predicates)) {
        System.out.printf("edwin ReduceExpressionsRule project do something \n");
        call.transformTo(
            call.builder()
  ...
  protected static boolean reduceExpressions(RelNode rel, List<RexNode> expList,
    ...
    System.out.printf("edwin cbo reduceExpressions reduced is %b, simplified is %b \n",
            reduced, simplified);
  ...
  protected static boolean reduceExpressionsInternal(RelNode rel, List<RexNode> expList,
    ...
    //constExps 主要是find RexInputRef的变量。这种变量才能拿来替换
    ...
    if (constExps.isEmpty() && removableCasts.isEmpty()) {
      System.out.printf("edwin reduceExpressionsInternal constExps is %s, constants is %s \n",
              constExps.toString(), constants.toString());
      return false;
    }
    System.out.printf("edwin reduceExpressionsInternal constExps is %s, constants is %s \n",
            constExps.toString(), constants.toString());
    ...
    System.out.printf("edwin constExps.isEmpty() is not null \n");
  ...
  private static <C extends RexNode> void gatherConstraint(Class<C> clazz,
    ...
    System.out.printf("edwin gatherConstraint excludeSet is %s\n", excludeSet.toString());
    System.out.printf("edwin gatherConstraint map is %d\n", map.size());
    ...
    if (existedValue == null) {
      ...
      System.out.printf("edwin gatherConstraint map put left is %s, class is %s\n",
              left.toString(), left.getClass());
      System.out.printf("edwin gatherConstraint map put constant is %s, class is %s\n",
              constant.toString(), constant.getClass());
