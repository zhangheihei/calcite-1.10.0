...
public class RelOptRuleOperand {
  ...
  private final Class<? extends RelNode> clazz; //保存当前关系表达式的类型
  ...
  public boolean matches(RelNode rel) {
    //检测传入的rel是否是该operand中保存的类型
    if (!clazz.isInstance(rel)) {
      return false;
    }

    //如果trait为null, null != null 为false
    if ((trait != null) && !rel.getTraitSet().contains(trait)) {
      return false;
    }
    //每种关系表达式都有自己的predicate类,目前已经见过的
    //PROJECT_PREDICATE,FILTER_PREDICATE
    //PROJECT的规则是 return false
    //FILTER 也是return false 以上都是针对熊猫的SQL
