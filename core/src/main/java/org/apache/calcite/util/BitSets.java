...
public final class BitSets {
  ...
  public static SortedMap<Integer, BitSet> closure(
    ...
    if (equivalence.size() < length
      ...
      System.out.printf("edwin BitSets SortedMap check \n");
  ...
  private static class Closure {
    ...
    private BitSet computeClosure(int pos) {
      ...
      o = (BitSet) b.clone();
      //此处是为了检查 当前索引下一位,还有没有占位
      int i = b.nextSetBit(pos + 1);
      if (i < 0) {
        System.out.printf("edwin computeClosure b.nextSetBit as -1, %s, pos:%d\n",
                b.toString(), pos);
      }
      ...
      if (i < 0) {
        System.out.printf("edwin computeClosure o.nextSetBit as -1, %s, pos:%d \n",
                o.toString(), pos);
      }
      for (; i >= 0; i = b.nextSetBit(i + 1)) {
        closure.put(i, o);
